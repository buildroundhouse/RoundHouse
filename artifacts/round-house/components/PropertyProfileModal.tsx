...
interface Props {
  ...
  /**
   * Carried over from the avatar profile so the wider call sites compile.
   * Properties don't have a counterpart skin; this is currently unused.
   */
...
export function PropertyProfileModal({
  ...
  // The avatar profile feeds off `useGetUserById`. The property variant
  // pulls the same shape from `useGetProperty` so we can reuse the rest
  // of the rendering code with minimal changes; `clerkId` stays here as
  // a stable null so connection / message branches simply skip.
  ...
  const queryKey = propertyId
    ...
    : ["/api/properties/none"];
  const { data: propertyData, isFetching } = useGetProperty(propertyId ?? 0, {
    query: { enabled: visible && propertyId != null, queryKey },
  });

  // Adapter: shape the property response into the user-shaped object the
  // avatar profile renders against. Fields that don't apply to a property
  // (sponsorBrandName, services, intake snapshot, etc.) are left null and
  // the existing conditional render branches simply hide. The `any` cast
  // is intentional — this scaffolds the entire avatar profile against a
  // property; we'll prune & retype the unused branches as we adjust.
  const data = useMemo<any>(() => {
    if (!propertyData) return undefined;
    return {
      user: {
        clerkId: `property:${propertyData.id}`,
        name: propertyData.name ?? "",
        username: propertyData.address ?? propertyData.type ?? "",
        avatarUrl: propertyData.coverPhotoUrl ?? null,
        updatedAt: null,
  ...
  // No team data for properties — keep the call site shape so the rest of
  // the component can consume an empty value without branching.
  const teamData: any = undefined;
  void getGetUserTeamQueryKey;
  void useGetUserTeam;
  ...
  const avatarUri = resolveStorageUrl(user?.avatarUrl ?? null, user?.updatedAt ?? null);
    ...
    <Modal visible={visible} animationType="slide" onRequestClose={onClose}>
      <View style={[styles.root, { backgroundColor: colors.background }]}>
        ...
        >
          ...
          {/* #643 — Message affordance in the public profile header.
              Hidden for self-views and while the profile is loading; the
              inbox screen handles team-up gating with a 403 banner if the
              viewer hasn't been accepted as a connection. */}
          {user && !isSelf && clerkId ? (
            <Pressable
              onPress={() => {
                const href = messageHrefFor({
                  clerkId,
                  counterpartOutwardAccountId: counterpartOutwardAccountId ?? null,
                });
                if (!href) return;
                onClose();
            ...
            >
              ...
              <Text style={[styles.headerMessageText, { color: colors.foreground }]}>
                Message
              </Text>
            </Pressable>
          ) : (
          ...
          >
              ...
              <View style={{ paddingHorizontal: 16, marginTop: 8 }}>
                {connection && connection.status === "pending" ? (
                      ...
                      const pillContent = (
                        <>
                          ...
                          <Text
                            ...
                            ellipsizeMode="tail"
                          >
                            Connected as {KIND_LABEL[connection.kind as ConnectionKind]}
                            {(() => {
                  ...
                  <Pressable
                    ...
                    style={[
                      styles.primaryBtn,
                      { backgroundColor: colors.primary ?? "#1877F2", opacity: connect.isPending ? 0.6 : 1 },
                    ]}
                  >
            ...
            {!user.email && !user.phone && !user.cellPhone && !user.officePhone && !user.address && !user.website && !user.instagram && !company && !isSelf && !connection ? (
              <Text style={[styles.privacyNote, { color: colors.mutedForeground }]}>
                Contact details unlock once you connect — or when this pro has chosen to make them public.
              </Text>
            ) : null}
