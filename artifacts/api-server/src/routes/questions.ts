...
router.patch(
  ...
  async (req, res): Promise<void> => {
    ...
    if (existing.resolutionState) {
      res.status(409).json({
        error:
          "Open this item in the Resolution Center to preserve its discussion and closeout history",
      });
      return;
    }
    if (
      (confirm === true || complete === true) &&
      existing.userClerkId !== userId
    ) {
    ...
    const [row] = await db
      ...
      .set(updates)
      .where(
        and(eq(questionsTable.id, id), isNull(questionsTable.resolutionState)),
      )
      .returning();

    if (!row) {
      res.status(409).json({
        error: "This resolution changed. Open it in the Resolution Center.",
      });
      return;
    }
