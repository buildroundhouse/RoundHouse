...
The Daily Grind rail tab opens the dedicated planner. Users add or remove references to existing list items; no source item is copied or marked completed merely by planning it. Completed source items no longer appear as unfinished plan items.
...
The sheet includes today's appointments and timed reminders, overdue unfinished reminders, and actionable Resolutions. Existing reminder and Resolution destinations retain ownership of their records. Plans and checklist state store source IDs per person and local date on the device. Existing shopping/custom lists remain the current on-device list store; cross-device list/plan synchronization is not implemented.

The Calendar now supplies the confirmed appointment feed. No weather integration is present, so the date line reports weather as unavailable rather than fabricating a forecast. Tasks & Lists opens the existing editor. The separate full Tasks / Lists master-index redesign is outside this change.
